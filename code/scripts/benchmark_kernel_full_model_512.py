...
for img_size in [256, 512]:
    model = build_kernel_model("fpn_mamba_full", dropout=0.3, fpn_channels=256, d_state=16,
                                freeze_stages=2, img_size=img_size).to(device).train()
    ...
    print(f"fpn_mamba_full @ {img_size}px: {step_ms:.1f} ms/step  {throughput:.1f} samples/s  "
