...
class SelfAttention2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # need_weights=False lets PyTorch dispatch to its fused/memory-efficient attention kernel
        # (avoids materializing the full LxL attention matrix) -- same math, ~O(L) memory instead
        # of O(L^2). Without this, nn.MultiheadAttention defaults to need_weights=True, which forces
        # the naive path and is what made 512px infeasible (~32GB OOM at L=16384 tokens).
