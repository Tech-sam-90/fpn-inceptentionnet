...
Table~\ref{tab:efficiency} summarises the efficiency profile of both models. The $16.5\times$ parameter gap (17.8M vs.\ 1.1M) requires context: approximately 9.1M of FPN-Mamba's parameters reside in the EfficientNet-B2 ImageNet-pretrained backbone; the FPN neck, Mamba blocks, and classification head account for the remaining $\approx8.7$M. InceptentionNet, by contrast, trains its 1.1M parameters from scratch. The more informative comparison is therefore the FLOPs ratio: 33.1 vs.\ 19.3 GFLOPs ($1.7\times$), reflecting that the Mamba scans operate in linear time $O(L)$ and add relatively little compute per token. Throughput follows accordingly: 112.2 FPS vs.\ 224.3 FPS ($2\times$ slower). Both speeds are well above real-time requirements for clinical screening. Wall-clock training time is similarly modest: FPN-Mamba completes 5-fold CV in approximately 90 minutes on an A100 vs.\ 45 minutes for InceptentionNet, a $2\times$ difference despite the $16.5\times$ parameter gap, because the large batch size (64 vs.\ 8) reduces gradient steps per epoch by $8\times$ and the pretrained backbone converges in fewer effective epochs. During training, \texttt{freeze\_layers=3} keeps the first three EfficientNet-B2 stages fixed, leaving $\approx13.1$M parameters actively updated.
