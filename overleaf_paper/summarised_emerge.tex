...
\midrule
\multicolumn{6}{l}{\textit{Bootstrap 95\% CI (InceptentionNet vs.\ FPN-Mamba, Wilcoxon $p=0.0625$)}} \\
InceptentionNet AUC & \multicolumn{5}{l}{[75.05, 89.15]\%} \\
FPN-Mamba AUC       & \multicolumn{5}{l}{[98.75, 99.80]\% \quad (non-overlapping)} \\
InceptentionNet F1  & \multicolumn{5}{l}{[59.12, 63.80]\%} \\
FPN-Mamba F1        & \multicolumn{5}{l}{[89.92, 98.16]\% \quad (non-overlapping)} \\
\bottomrule
\end{tabular}
\end{table}
...
%----------------------------------------------------------------------
FPN-Mamba achieves the best performance across all five metrics against every baseline. Against InceptentionNet ($+16.38$ pp AUC, $+33.08$ pp F1), against EfficientNet-B2$+$GAP ($+4.54$ pp AUC, $+14.61$ pp F1), and against the strongest competitor ResNet-50 ($+1.44$ pp AUC, $+7.24$ pp F1). These gains compound clinically: FPN-Mamba misses approximately 4 of 131 MB cases versus 7 for ResNet-50 and 31 for InceptentionNet---a 43\% reduction relative to the best alternative and a 7.75$\times$ reduction relative to the domain-specific baseline. Fang et al.\ \cite{fang2025} identified small lesions, low contrast, and atypical location as the primary sources of false negatives in single-scale models; FPN-Mamba addresses exactly these failure modes by retaining a full pyramid from $P_2$ to $P_5$, so a tumor invisible at the coarsest scale remains visible at a finer level, with cross-scale Mamba interpreting it in broader anatomical context. The fact that even ResNet-50 (a strong general-purpose pretrained baseline) falls 1.44 pp short in AUC underscores that multi-scale feature aggregation, not merely backbone capacity, drives the improvement.
...
FPN-Mamba integrates an EfficientNet-B2 backbone, an FPN with Bidirectional Mamba SSMs at each level, cross-scale Mamba context modeling, Generalized Mean Pooling, and Squeeze-and-Excitation channel attention. Evaluated against three baselines trained under identical conditions, FPN-Mamba achieves AUC $99.35\%$---outperforming InceptentionNet ($82.97\%$, $+16.38$ pp), EfficientNet-B2$+$GAP ($94.81\%$, $+4.54$ pp), and ResNet-50 ($97.91\%$, $+1.44$ pp)---with non-overlapping bootstrap CIs and a Wilcoxon test at the minimum achievable $p$ for $n=5$ folds confirming consistent improvement. The ablation demonstrates that FPN is the dominant component, with Mamba and GeM$+$SE providing complementary sensitivity and balance gains. A 3-class supplementary benchmark (AUC $99.50\%$) confirms the architecture generalises beyond the binary task. Future work includes multi-institution external validation, Grad-CAM visualisations across pyramid levels for interpretability, and further disease-specific MRI case studies.
