...
Table~\ref{tab:efficiency} summarises the efficiency profile of both models. FPN-Mamba has 17.8M parameters vs.\ 1.1M for InceptentionNet ($16.5\times$ larger), but the FLOPs ratio is far more favourable: 33.1 vs.\ 19.3 GFLOPs ($1.7\times$). The FLOPs gap is modest because the EfficientNet-B2 backbone---which accounts for the bulk of compute---is shared in spirit, and the Mamba scans operate in linear time. Throughput reflects this: 112.2 FPS vs.\ 224.3 FPS, approximately $2\times$ slower. Both speeds are well above real-time requirements for clinical screening; in a setting processing at most a few hundred scans per session, the $+16.38$ pp AUC gain easily justifies the added parameter and compute cost. During training, \texttt{freeze\_layers=3} keeps the first three EfficientNet-B2 stages fixed, leaving $\approx13.1$M parameters actively trained.

\begin{table}[h]
\centering
\caption{Efficiency comparison at $224\times224$ input (CPU profiling via \texttt{thop}).}\label{tab:efficiency}
\small
\begin{tabular}{lrrrr}
\toprule
\textbf{Model} & \textbf{Params (M)} & \textbf{GFLOPs} & \textbf{MACs} & \textbf{FPS} \\
\midrule
InceptentionNet & 1.1 & 19.3 & 9.65G & 224.3$\pm$4.7 \\
FPN-Mamba       & 17.8 & 33.1 & 16.57G & 112.2$\pm$0.82 \\
\midrule
Ratio (FPN / Incept.) & $16.5\times$ & $1.7\times$ & $1.7\times$ & $0.5\times$ \\
\bottomrule
\end{tabular}
\end{table}
