...
Table~\ref{tab:per_fold} reports per-fold FPN-Mamba results. Variance is moderate (F1 std 5.49\%, driven mainly by Fold 4 at 85.71\%), with the remaining four folds all above 94\% F1, indicating stable training despite the small dataset size. Fold 2's perfect scores (100\% across all metrics) are plausible given only $\approx$26 MB validation images per fold: when all cases in a fold lie well within the learned decision boundary, zero errors occur; this is consistent with the overall low AUC variance (std 0.68\%) and not indicative of data leakage.
...
To test generalisation, we evaluated FPN-Mamba on the 3-class benchmark of Abiwinanda et al.\ \cite{abiwinanda2019} (meningioma, glioma, pituitary; Cheng et al.\ dataset) using 3-fold CV, which the larger and balanced Cheng et al.\ dataset supports without the small-fold instability risk present in the imbalanced MB task. Table~\ref{tab:multiclass_partial} reports results. With mean AUC 99.50\% and F1 94.65\%, the architecture generalises well beyond the binary MB task.
...
Table~\ref{tab:efficiency} summarises the efficiency profile of both models. The $16.5\times$ parameter gap (17.8M vs.\ 1.1M) requires context: approximately 9.1M of FPN-Mamba's parameters reside in the EfficientNet-B2 ImageNet-pretrained backbone; the FPN neck, Mamba blocks, and classification head account for the remaining $\approx8.7$M. InceptentionNet, by contrast, trains its 1.1M parameters from scratch. The more informative comparison is therefore the FLOPs ratio: 33.1 vs.\ 19.3 GFLOPs ($1.7\times$), reflecting that the Mamba scans operate in linear time $O(L)$ and add relatively little compute per token. Throughput follows accordingly: 112.2 FPS vs.\ 224.3 FPS ($2\times$ slower). Both speeds are well above real-time requirements for clinical screening. Wall-clock training is $2\times$ longer (90 vs.\ 45 min for 5-fold CV on A100) despite the $16.5\times$ parameter gap, because batch size 64 cuts gradient steps per epoch by $8\times$ and the pretrained backbone converges in fewer epochs. \texttt{freeze\_layers=3} keeps the first three EfficientNet-B2 stages fixed, leaving $\approx13.1$M parameters actively updated.
...
All quantitative claims rest on a single-institution 761-image dataset; absolute numbers may not generalise to the broader clinical population, though the large relative gaps between methods are robust to this caveat. Public MB-labeled MRI data is scarce ($\approx$500 US cases per year), with TCIA and OpenNeuro restricting access or lacking paired annotations. A specificity check on the Nickparvar dataset \cite{nickparvar2021} (10\,560 images, four non-MB classes) reveals 95.6\% specificity on normal tissue but elevated false-positive rates for glioma (37.2\%) and pituitary tumors (42.8\%). These rates are clinically explicable---both tumor types share posterior fossa anatomy and mass-effect characteristics with MB---but represent a genuine deployment risk: a model trained exclusively on one institution's MB images may not reliably reject visually similar tumor types encountered in practice. Resolving this requires multi-institution co-training with all relevant tumor classes represented. The heavier MB-specific augmentation is a potential confound; since all models received identical augmentation, any effect is symmetric, but a dedicated augmentation ablation is deferred to future work.

\paragraph{Conclusion.}
FPN-Mamba integrates an EfficientNet-B2 backbone, an FPN with Bidirectional Mamba SSMs at each level, cross-scale Mamba context modeling, Generalized Mean Pooling, and Squeeze-and-Excitation channel attention. Evaluated against three baselines trained under identical conditions, FPN-Mamba achieves AUC $99.35\%$---outperforming InceptentionNet ($82.97\%$, $+16.38$ pp), EfficientNet-B2$+$GAP ($94.81\%$, $+4.54$ pp), and ResNet-50 ($97.91\%$, $+1.44$ pp)---with non-overlapping bootstrap CIs and a Wilcoxon test at the minimum achievable $p$ for $n=5$ folds confirming consistent improvement. Elevated false-positive rates on visually similar tumor types (glioma 37.2\%, pituitary 42.8\%) remain a deployment limitation requiring multi-institution data to resolve. The ablation demonstrates that FPN is the dominant component, with Mamba and GeM$+$SE providing complementary sensitivity and balance gains. A 3-class supplementary benchmark (AUC $99.50\%$) confirms the architecture generalises beyond the binary task. Future work includes multi-institution external validation, prospective clinical validation as a radiologist decision-support tool, Grad-CAM visualisations for interpretability, and further disease-specific MRI case studies.
